...
const SvgIconRendered = forwardRef<SVGSVGElement, SvgIconRenderedProps>(({icon, svgText, cache = true, width, height, style = {}, ...props}, ref) => {
...
const CustomIcon = forwardRef<SVGSVGElement | HTMLImageElement | HTMLDivElement, IconProps>(({ icon, className, style = {}, cache = true, ...props }, ref) => {
...
export const Icon = forwardRef<SVGSVGElement, IconProps>(({ children, icon, cache = true, ...props }, ref) => {
    ...
    useAsyncLayoutEffect(async () => {
        ...
        if (cache && fromCache) {
            ...
            const svgText = await fetch(browser.runtime.getURL(`/assets/icons/${family}/${iconName}.svg`)).then(r => r.text());
            setSvgText(svgText);
        }
    }, [icon]);
