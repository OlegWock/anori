...
export async function migrateFromLegacy(): Promise<void> {
  ...
  newData[SCHEMA_VERSION_KEY] = anoriVersionedSchema.currentVersion;
  ...
  console.log(
    `[Storage] Migrated from legacy storage (v${allData.storageVersion ?? 0}) to schema v${anoriVersionedSchema.currentVersion}`,
  );
}
