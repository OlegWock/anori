...
export const addUniversalDnrRules = async () => {
  ...
  return browser.declarativeNetRequest.updateDynamicRules({
    ...
    removeRuleIds: [MODIFY_RESPONSE_RULE_ID, MODIFY_REQUEST_RULE_ID],
  });
};
...
export const clearBrowsingData = async (url: string) => {
  if (browser.browsingData) {
    const options =
      X_BROWSER === "firefox" ? { hostnames: [new URL(url).hostname] } : { origins: [new URL(url).origin] };
    const dataToRemove =
      X_BROWSER === "firefox"
        ? {
            cache: true,
...
export const plantWebRequestHandler = () => {
  ...
  browser.webRequest.onHeadersReceived.addListener(webResponseHandler, { urls: ["<all_urls>"] }, [
    "responseHeaders",
    "blocking",
  ]);
  browser.webRequest.onBeforeSendHeaders.removeListener(webRequestHandler);
  browser.webRequest.onBeforeSendHeaders.addListener(webRequestHandler, { urls: ["<all_urls>"] }, [
    "requestHeaders",
    "blocking",
  ]);
