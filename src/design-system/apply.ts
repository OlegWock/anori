...
// it onto the document root as `--ds-*` CSS variables. Returns the palette so the caller can also derive
// the legacy `--accent`/`--background`/… vars from it (components migrate to `--ds-*` gradually).
